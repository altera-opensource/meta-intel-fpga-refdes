...
do_compile:agilex5_dk_a5e013bm16aea() {
	...
	export HYP_BUILD="${@bb.utils.contains('HYP_BUILD', '1', '1', '0', d)}"
	if [[ "${HYP_BUILD}" = "1" ]]; then
		mkimage -f "${WORKDIR}/agilex5_013b_uboot_script_xen.its" ${WORKDIR}/boot.scr.xen.uimg
	fi
}
...
do_compile:agilex5_mk_a5e065bb32aes1() {
	mkimage -f "${WORKDIR}/uboot_script.its" ${WORKDIR}/boot.scr.uimg
        export HYP_BUILD="${@bb.utils.contains('HYP_BUILD', '1', '1', '0', d)}"
        if [[ "${HYP_BUILD}" = "1" ]]; then
                mkimage -f "${WORKDIR}/agilex5_modular_uboot_script_xen.its" ${WORKDIR}/boot.scr.xen.uimg
        fi
}
	...
	if [[ "${MACHINE}" == "arria10" ]]; then
		...
		if [[ "${HYP_BUILD}" = "1" ]]; then
			...
			if [[ "${MACHINE}" == "agilex5_dk_a5e013bm16aea" ]]; then
				install -m 0755 ${WORKDIR}/agilex5_013b_uboot_xen.txt ${DEPLOYDIR}/u-boot_xen.txt
			elif [[ "${MACHINE}" == "agilex5_mk_a5e065bb32aes1" ]]; then
				install -m 0755 ${WORKDIR}/agilex5_modular_uboot_xen.txt ${DEPLOYDIR}/u-boot_xen.txt
			else
